...
try{
const res = await axios.get(`http://localhost:8080/api/resumes/user/${USER_ID}`);
setResumes(res.data);
}catch{
...
try{
await axios.put(
`http://localhost:8080/api/resumes/${editingResume.id}`,
editingResume
);
alert("Updated ✅");
...
window.open(`http://localhost:8080/api/resumes/${id}/download`);
};
